...
- Google sign-in is live: the Google Cloud OAuth client has one fresh persistent secret stored only in Supabase Auth, the audience is in production, and a full production E2E completed from `/login` through Google consent to both `/root` and `/admin`. The old Google secret was disabled and permanently deleted in Google Cloud on 2026-09-04. Apple remains disabled until the pending Apple Developer Program purchase is completed and credentials are created.
