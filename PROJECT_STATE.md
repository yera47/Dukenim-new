...
The next release candidate adds `/root/stores/[id]`: audited superadmin product corrections and reversible visibility controls, plus store-scoped orders/categories/audit context. It does not expose permanent deletion, credential access, arbitrary SQL or direct stock mutation. TypeScript, 47 tests and the 56-route build pass; production deployment/visual verification remain pending.
