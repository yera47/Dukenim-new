...
export function RegisterForm({ socialRegistration = false, plan = "standard", billing = "month" }: { socialRegistration?: boolean; plan?: PublicPlan; billing?: "month" | "year" }) {
  ...
  if (socialRegistration) {
    return <form action={action} className="mt-8 space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-extrabold">Ваше имя<input className="input mt-2" name="name" required autoComplete="name" placeholder="Серик" /></label><label className="text-sm font-extrabold">Название бизнеса<input className="input mt-2" name="business" required placeholder="Серик Шоп" /></label></div>
      <Acceptance />
  ...
  const socialNext = `/register?social=1&plan=${plan}&billing=${billing}`;
  return <>{hasSocialAuthProviders && <><div className="mt-8"><SocialAuthButtons mode="register" next={socialNext} /></div><div className="my-6 flex items-center gap-3 text-xs font-bold text-[var(--ink-40)]"><span className="h-px flex-1 bg-[var(--line)]"/>или по email<span className="h-px flex-1 bg-[var(--line)]"/></div></>}<form action={action} className="space-y-4">
    <input type="hidden" name="plan" value={plan} />
    <input type="hidden" name="billing" value={billing} />
    <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-extrabold">Ваше имя<input className="input mt-2" name="name" required autoComplete="name" placeholder="Серик" /></label><label className="text-sm font-extrabold">Название бизнеса<input className="input mt-2" name="business" required placeholder="Серик Шоп" /></label></div>
