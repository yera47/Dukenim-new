...
export default function Home() {
  return <main className={styles.home}>
    ...
    <section className={styles.hero}><small>Ваш магазин. В одном пространстве.</small><h1>Меньше переписки.<br/><span>Больше порядка.</span></h1><p>Покупатель выбирает и заказывает по одной ссылке.<br/>Вы управляете каталогом, заказами и бизнесом в Dukenim.</p><div className={styles.actions}><Link href="/register" className={styles.primary}>Создать магазин <ArrowRight size={17}/></Link><Link href="/s/demo-shop" className={styles.secondary}>Посмотреть пример</Link></div><small>7 дней бесплатно · Без банковской карты</small><div className={styles.prompt}><b><Sparkles size={20}/> С чего начнём ваш магазин?</b><p>После регистрации откроется AI Studio. Опишите магазин, создайте основу каталога и добавьте товары в одном рабочем пространстве.</p><div><Link href="/register"><Package size={15}/>Создать каталог</Link><a href="#ai"><Sparkles size={15}/>Познакомиться с AI Studio</a><a href="#system">Как это работает <ArrowRight size={15}/></a></div></div></section>
    <PromoFilm/>
