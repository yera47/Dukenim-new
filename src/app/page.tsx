...
export default function Home() {
  return <main className={styles.home}>
    ...
    <PromoFilm/>
    <section id="products" className={styles.section}><div className={styles.heading}><small>ОТ ВЫБОРА ДО ЗАКАЗА</small><h2>Покупателю — магазин.<br/>Вам — ясная картина.</h2><p>Витрина и рабочий кабинет связаны между собой. Не нужно переносить каждый заказ из переписки в таблицу.</p></div><RecordedCommerce/><div className={styles.columns}>{[["Каталог","Фото, цены, варианты и наличие. Всё необходимое для выбора."],["Заказы","Контакты и состав заказа в одном месте. Подтверждайте и меняйте статус."],["Управление","Остатки, клиенты и аналитика входят в единый тариф «Каталог»."]].map(([title,text])=><article key={title}><h3>{title}</h3><p>{text}</p></article>)}</div></section>
    ...
    <section id="system" className={styles.section}><div className={styles.heading}><small>ПОНЯТНЫЙ СТАРТ</small><h2>Один шаг за другим.</h2></div><div className={styles.columns}>{[["Создайте аккаунт","Войдите через Google или email и выберите тип бизнеса. Единый тариф «Каталог» уже включает все функции."],["Соберите каталог","В AI Studio опишите свой бизнес, сохраните основу и разделы каталога. Добавьте фотографии, цены и условия получения."],["Проверьте и поделитесь","Откройте витрину как покупатель, проверьте путь заказа и разместите ссылку в Instagram."]].map(([title,text],i)=><article key={title}><span className={styles.number}>0{i+1}</span><h3>{title}</h3><p>{text}</p></article>)}</div></section>
    <PricingSection/><MarketingFaq/>
    <section id="security" className={styles.trust}><h2>Ваши данные — под вашим контролем.</h2><p>Доступ к магазину проверяется на сервере. Данные разных магазинов разделены. Dukenim не хранит данные банковских карт; онлайн-оплата для покупателей подключается отдельно.</p></section>
