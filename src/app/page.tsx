...
import { CommerceMotion } from "@/components/marketing/commerce-motion";
...
export default function Home() {
  return <main className={styles.home}>
    ...
    <section className={styles.hero}><small>Ваш магазин. В одном пространстве.</small><h1>Меньше переписки.<br/><span>Больше порядка.</span></h1><p>Покупатель выбирает и заказывает по одной ссылке.<br/>Вы управляете каталогом, заказами и бизнесом в Dukenim.</p><div className={styles.actions}><Link href="/register" className={styles.primary}>Создать магазин <ArrowRight size={17}/></Link><Link href="/s/demo-shop" className={styles.secondary}>Посмотреть пример</Link></div><small>7 дней бесплатно · Без банковской карты</small><div className={styles.prompt}><b><Sparkles size={20}/> С чего начнём ваш магазин?</b><p>Выберите тип бизнеса, создайте каталог и добавьте первый товар.</p><div><Link href="/register"><Package size={15}/>Создать каталог</Link><a href="#ai"><Sparkles size={15}/>Познакомиться с AI Studio</a><a href="#system">Как это работает <ArrowRight size={15}/></a></div></div></section>
    <section id="products" className={styles.section}><div className={styles.heading}><small>ОТ ВЫБОРА ДО ЗАКАЗА</small><h2>Покупателю — магазин.<br/>Вам — ясная картина.</h2><p>Витрина и рабочий кабинет связаны между собой. Не нужно переносить каждый заказ из переписки в таблицу.</p></div><CommerceMotion/><div className={styles.columns}>{[["Каталог","Фото, цены, варианты и наличие. Всё необходимое для выбора."],["Заказы","Контакты и состав заказа в одном месте. Подтверждайте и меняйте статус."],["Управление","На «Бренде» — остатки, клиенты и аналитика, связанные с продажами."]].map(([title,text])=><article key={title}><h3>{title}</h3><p>{text}</p></article>)}</div></section>
    <section id="ai" className={styles.section}><div className={styles.heading}><small>AI STUDIO</small><h2>Помощник для<br/>конкретных задач.</h2><p>Тексты витрины, структура каталога и баннеры. Вы проверяете результат и решаете, что использовать.</p></div><div className={styles.conversation}><div className={styles.question}>Помоги описать новую коллекцию для моего магазина.</div><div className={styles.answer}><Sparkles size={23}/><div><b>Начнём с того, что важно покупателю.</b><p>Расскажите о товарах, аудитории и стиле бренда. AI Studio подготовит заголовок, описание и текст кнопки для вашей витрины.</p><small>Пример сценария · результат требует проверки</small></div></div><Link href="/register" className={styles.primary}>Начать с AI Studio <ArrowRight size={16}/></Link><p className={styles.support}><MessageCircle size={18}/>Нужен человек? Поддержка доступна отдельно из кабинета и AI Studio.</p></div></section>
    <section id="system" className={styles.section}><div className={styles.heading}><small>ПОНЯТНЫЙ СТАРТ</small><h2>Один шаг за другим.</h2></div><div className={styles.columns}>{[["Создайте аккаунт","Войдите через Google или email. Выберите тип бизнеса и подходящий тариф."],["Соберите каталог","AI Studio подскажет первый шаг. Выберите оформление, добавьте товар и условия получения."],["Проверьте и поделитесь","Откройте витрину как покупатель, проверьте путь заказа и разместите ссылку в Instagram."]].map(([title,text],i)=><article key={title}><span className={styles.number}>0{i+1}</span><h3>{title}</h3><p>{text}</p></article>)}</div></section>
