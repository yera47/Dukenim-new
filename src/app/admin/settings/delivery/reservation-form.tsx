...
export function ReservationForm({enabled=false,hours=24,location}:{enabled?:boolean;hours?:number;location?:unknown}){
 ...
 {([['address','Адрес магазина'],['gisUrl','Ссылка на 2ГИС'],['yandexUrl','Ссылка на Яндекс Карты']] as const).map(([key,label])=><label key={key} className="block text-sm">{label}<input className="input mt-2" name={key} defaultValue={point?.[key]??""} maxLength={key.includes('Url')?1500:300}/></label>)}
