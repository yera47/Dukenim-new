...
export function ReservationForm({enabled=false,hours=24,location}:{enabled?:boolean;hours?:number;location?:unknown}){
 ...
 {([['address','Адрес магазина'],['gisUrl','Ссылка на 2ГИС'],['yandexUrl','Ссылка на Яндекс Карты или Навигатор']] as const).map(([key,label])=><label key={key} className="block text-sm">{label}<input className="input mt-2" name={key} defaultValue={point?.[key]??""} maxLength={key.includes('Url')?1500:300}/></label>)}
 <input type="hidden" name="preparation" value="После подтверждения готовности продавцом в разделе «Мои заказы»"/>
 <p className="text-sm text-neutral-500">Сначала проверьте наличие и подготовьте товар. Подтвердите готовность в заказе — только после этого покупатель увидит, что можно приезжать.</p>
