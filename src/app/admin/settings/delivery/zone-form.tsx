...
export function ZoneForm({ zone }: { zone: Zone }) {
  ...
  return <form action={action} className="card space-y-4 p-5">
    ...
    <fieldset disabled={pending} className="grid gap-4 sm:grid-cols-2">
      <label className="sm:col-span-2">Кто доставляет<select className="input mt-2" name="provider" value={provider} onChange={(event) => setProvider(event.target.value as Zone["provider"])}><option value="own">Своя доставка</option><option value="yandex">Курьер через Яндекс · магазин заказывает сам</option></select></label>
