...
import { orderCountLabel, orderStatusLabels } from "@/lib/order-display";
...
export default async function Admin() {
  ...
  const revenue = todayOrders.filter((order) => order.status !== "cancelled" && order.payment_status === "paid").reduce((sum, order) => sum + order.total, 0);
  const low = catalog.variants.filter((variant) => variant.stock_qty > 0 && variant.stock_qty <= 3);
  const paidOrders=orders.filter(order=>order.status!=="cancelled"&&order.payment_status==="paid");
  const average = paidOrders.length ? Math.round(paidOrders.reduce((sum, order) => sum + order.total, 0) / paidOrders.length) : 0;
  return <>
