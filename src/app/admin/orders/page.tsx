...
export default async function Orders(){
 ...
 return <><h1 className="text-3xl font-semibold">Заказы и бронирования</h1><p className="muted mt-2">Бронь становится продажей только после выдачи и получения оплаты.</p>{holds.error&&<p role="alert">Состояния брони не загрузились. Управление заказами временно скрыто.</p>}<div className="mt-6 grid gap-4">{orders.map(o=>{const hold=byId.get(o.id);return <article key={o.id} className="card flex flex-wrap items-start justify-between gap-4 p-5"><div><b>{hold?'Бронь':'Заказ'} #{o.order_number}</b><p className="muted mt-1 text-sm">{new Date(o.created_at).toLocaleString("ru-KZ")}</p>{planfixConnected&&!hold&&<PlanfixOrderSyncButton orderId={o.id} status={planfixStates.get(o.id)}/>}</div><span className="badge">{hold?'В магазине':o.source==="online"?"Онлайн":"В зале"}</span><strong>{money(o.total)}</strong>{!holds.error&&(hold?<ReservationControls key={hold.status} reservation={hold}/>:<OrderStatusForm key={`${o.id}-${o.status}`} id={o.id} status={o.status}/>)}</article>})}</div></>;
}
