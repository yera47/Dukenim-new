...
export function KaspiPayment({orderId,paid,invoiceSent,total,refunded=false}:{orderId:string;paid:boolean;invoiceSent:boolean;total:number;refunded?:boolean}){
 ...
 return <div className="mt-4 max-w-md rounded-xl border border-sky-200 bg-sky-50 p-4 text-sm"><b>Удалённая оплата Kaspi Pay · {money(total)}</b>
  {!paid&&<p className="mt-2">{invoiceSent?'Вы отметили отправку счёта.':'Откройте Kaspi Pay → Удалённая оплата → Выставить счёт. Телефон покупателя указан выше.'} Покупатель также может перейти по ссылке магазина из «Моих заказов» и сам ввести сумму. Переход по ссылке не подтверждает оплату — сверяйте поступление, сумму и номер заказа в Kaspi Pay.</p>}
  ...
  <form action={action} className="mt-3 space-y-3"><input type="hidden" name="orderId" value={orderId}/>
   ...
   <button name="action" value={paid?'refunded':'paid'} className="btn btn-cta text-xs" disabled={pending}>{pending?'Сохраняем…':paid?'Отметить возврат':'Подтвердить оплату и заказ'}</button>
