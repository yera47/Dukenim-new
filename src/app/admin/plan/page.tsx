...
export default async function PlanPage({ searchParams }: { searchParams: Promise<{ locked?: string; expired?: string; checkout?: string; portal?: string }> }) {
  ...
  let plan: Plan = "basic", period: string | null = null, trialEndsAt: string | null = null, status = "trial", polarCustomerId: string | null = null;
  if (process.env.NEXT_PUBLIC_SUPABASE_URL && tenantId) {
    ...
    if (tenant) { plan = tenant.plan; trialEndsAt = tenant.trial_ends_at; status = tenant.status; polarCustomerId = tenant.polar_customer_id; }
    period = subscriptionResult.data?.current_period_end ?? null;
  }
  const polarAvailable = isPolarConfigured("basic", "monthly");
  return <>
    ...
    <section className="card mt-6 overflow-hidden"><div className="bg-[var(--accent-dark)] p-7 text-white"><span className="badge bg-white/10 text-[var(--accent-bright)]">{status === "trial" ? "7 ДНЕЙ БЕСПЛАТНО" : "ТЕКУЩИЙ ПЛАН"}</span><h2 className="mt-4 text-4xl font-extrabold">{planName[plan]}</h2><p className="mt-2 text-white/65">{status === "trial" ? `После бесплатного периода: ${planPrice.basic.toLocaleString("ru-KZ")} ₸ в месяц. Списание не начнётся без подтверждения.` : period ? `Активен до ${new Date(period).toLocaleDateString("ru-KZ")}` : "Все функции Dukenim доступны в одном тарифе"}</p></div></section>
    <div className="mt-6"><article className="card flex max-w-2xl flex-col p-6 ring-2 ring-[var(--accent)]"><div className="flex items-start justify-between"><div><p className="data-label">ЕДИНЫЙ ТАРИФ</p><h2 className="mt-2 text-2xl font-extrabold">Каталог</h2></div><span className="badge">ВСЕ ФУНКЦИИ</span></div><p className="mt-4 text-3xl font-extrabold tabular">24 900 ₸<span className="text-sm font-medium text-[var(--ink-60)]"> / мес.</span></p><div className="my-6 h-px bg-[var(--line)]"/><div className="flex-1 space-y-3">{planFeatures.basic.map(feature => <p key={feature} className="flex gap-2 text-sm"><Check size={18} className="shrink-0 text-[var(--success)]"/>{feature}</p>)}</div><Link href="/admin/plan/checkout?plan=basic&billing=monthly" className="btn btn-primary mt-6 w-full">Перейти к оплате <ArrowRight size={17}/></Link></article></div>
    <p className="mt-5 flex items-center gap-2 text-sm text-[var(--ink-60)]"><ShieldCheck size={17}/> {polarAvailable ? "Оплата откроется у сертифицированного провайдера. Dukenim не хранит данные банковских карт." : "Сейчас доступна заявка на оплату по счёту. Тариф включается только после подтверждения платежа командой Dukenim."}</p>
  </>;
}
