...
export default async function PlanPage({ searchParams }: { searchParams: Promise<{ locked?: string; expired?: string }> }) {
  ...
  return <>
    ...
    <section className="card mt-6 overflow-hidden"><div className="bg-[var(--accent-dark)] p-7 text-white"><span className="badge bg-white/10 text-[var(--accent-bright)]">{status === "trial" ? "7 ДНЕЙ БЕСПЛАТНО" : "ТЕКУЩИЙ ПЛАН"}</span><h2 className="mt-4 text-4xl font-extrabold">{planName[status === "trial" ? nextPlan : plan]}</h2><p className="mt-2 text-white/65">{status === "trial" ? `После пробного периода — ${planPrice[nextPlan].toLocaleString("ru-KZ")} ₸/месяц. Автосписания нет.` : period ? `Активен до ${new Date(period).toLocaleDateString("ru-KZ")}` : "Управление подпиской"}</p></div></section>
    <div className="mt-6 grid gap-4 xl:grid-cols-3">{plans.map((item) => <article key={item} className={`card flex flex-col p-6 ${item === nextPlan ? "ring-2 ring-[var(--accent)]" : ""}`}><div className="flex items-start justify-between"><div><p className="data-label">ТАРИФ</p><h2 className="mt-2 text-2xl font-extrabold">{planName[item]}</h2></div>{item === nextPlan && <span className="badge">ВЫБРАН</span>}</div><p className="mt-4 text-3xl font-extrabold tabular">{planPrice[item].toLocaleString("ru-KZ")} ₸<span className="text-sm font-medium text-[var(--ink-60)]"> / мес.</span></p><div className="my-6 h-px bg-[var(--line)]"/><div className="flex-1 space-y-3">{planFeatures[item].map(feature => <p key={feature} className="flex gap-2 text-sm"><Check size={18} className="shrink-0 text-[var(--success)]"/>{feature}</p>)}</div><Link href={`/admin/plan/checkout?plan=${item}`} className={`btn mt-6 w-full ${item === nextPlan ? "btn-primary" : "btn-secondary"}`}>Оформить тариф <ArrowRight size={17}/></Link></article>)}</div>
