...
export default async function PlanPage({ searchParams }: { searchParams: Promise<{ locked?: string; expired?: string; checkout?: string; portal?: string }> }) {
  ...
  let plan: Plan = "basic", nextPlan: Plan = "standard", preferredBilling: "monthly" | "annual" = "monthly", period: string | null = null, trialEndsAt: string | null = null, status = "trial", polarCustomerId: string | null = null;
  if (process.env.NEXT_PUBLIC_SUPABASE_URL && tenantId) {
    ...
    if (tenant) { plan = tenant.plan; nextPlan = tenant.next_plan ?? "standard"; preferredBilling = tenant.preferred_billing_period; trialEndsAt = tenant.trial_ends_at; status = tenant.status; polarCustomerId = tenant.polar_customer_id; }
