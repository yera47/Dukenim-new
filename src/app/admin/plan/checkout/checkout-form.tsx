...
export function CheckoutForm({ plan, polarAvailable = false, initialPeriod = "monthly" }: { plan: Plan; polarAvailable?: boolean; initialPeriod?: "monthly" | "annual" }) {
  const [accepted, setAccepted] = useState(false); const [pending, setPending] = useState(false); const [message, setMessage] = useState("");
  const [period, setPeriod] = useState<"monthly" | "annual">(initialPeriod); const [promoCode, setPromoCode] = useState("");
