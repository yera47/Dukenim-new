...
type Design = { brandColor?: string; templateKey: string; paletteKey: string; heroTitle: string; heroSubtitle: string; heroCtaLabel: string; rationale: string };
    ...
    <StudioConversation enabled={enabled&&!pending} onBanner={imageEnabled?message=>{setIntent("banner");setBrief(message);void createDraft({intent:"banner",brief:message});}:undefined} onTask={task=>{setIntent(task.intent);setBrief(task.brief);void createDraft(task);}}>
      ...
      <aside className={styles.preview} aria-label="Результат и создание магазина">
        ...
        <div className={styles.result}>
          ...
          design ? <>{generationId && <iframe title="Предпросмотр предложения AI" className="h-[620px] w-full rounded-xl border" src={`/store-preview?generation=${encodeURIComponent(generationId)}`}/>}<div className={styles.designMeta}><span>{templateNames[design.templateKey] ?? design.templateKey}</span><span>{paletteNames[design.paletteKey] ?? design.paletteKey}</span>{design.brandColor&&<span>Акцент: {design.brandColor}</span>}</div><div className={styles.designHero}><small>ГЛАВНЫЙ ЭКРАН</small><h3>{design.heroTitle}</h3><p>{design.heroSubtitle}</p><b>{design.heroCtaLabel}</b></div><p>{design.rationale}</p><p>{designSaved ? "Оформление применено. Откройте витрину и проверьте результат глазами покупателя." : "AI подготовил вариант, но ещё ничего не изменил. Фотография сохранится; акцентный цвет будет таким, как в предпросмотре."}</p><button type="button" className="btn btn-primary" disabled={!generationId || designSaving || designSaved || pending} onClick={saveDesign}>{designSaving ? "Применяем…" : designSaved ? "Оформление применено" : "Применить оформление"}</button>{designSaved && <Link href={`/s/${slug}`} target="_blank" rel="noopener noreferrer">Открыть обновлённую витрину →</Link>}</> :
