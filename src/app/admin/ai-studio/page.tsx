...
export default async function AiStudioPage() {
  ...
  const savedDesign = client ? await client.from("ai_studio_generations").select("id,output").eq("tenant_id", tenantId!).eq("intent", "store_design").order("created_at", {ascending:false}).order("id", {ascending:false}).limit(1).maybeSingle() : null;
