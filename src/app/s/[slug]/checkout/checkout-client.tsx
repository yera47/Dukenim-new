  ...
  return <main className="container py-10">
    ...
    <div className="mx-auto mt-8 max-w-3xl">
      ...
      <section className="card p-6 md:p-9">
        {step === 1 && <><h1 className="text-3xl font-semibold">Ваши контакты</h1><p className="muted mt-2">Регистрация не нужна — только данные для подтверждения заказа.</p><div className="mt-7 grid gap-4"><input value={name} onChange={(event) => setName(event.target.value)} className="input" maxLength={80} autoComplete="name" placeholder="Ваше имя"/><input value={phone} onChange={(event) => setPhone(event.target.value)} className="input" maxLength={30} type="tel" inputMode="tel" autoComplete="tel" placeholder="+7 777 000 00 00"/></div>{minOrder > 0 && <p className="muted mt-4 text-sm">Минимальная сумма заказа: {money(minOrder)}</p>}</>}
