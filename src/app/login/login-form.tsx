...
export function LoginForm({next=""}:{next?:string}) {
  ...
  return <>{hasSocialAuthProviders && <><div className="mt-8"><SocialAuthButtons mode="login" next={next||"/auth/continue"} /></div><div className="my-6 flex items-center gap-3 text-xs font-bold text-[var(--ink-40)]"><span className="h-px flex-1 bg-[var(--line)]"/>или по email<span className="h-px flex-1 bg-[var(--line)]"/></div></>}<form action={action} className="space-y-5">
