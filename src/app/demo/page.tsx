...
export default function DemoPage() {
  return <main className="min-h-screen bg-white text-neutral-900"><div className="mx-auto max-w-6xl px-6 py-10">
    <nav className="flex justify-between text-sm"><Link href="/">← Dukenim</Link><Link href="/register">Создать магазин →</Link></nav>
    ...
    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{launchVerticals.map(({id:vertical})=>{const preset=nichePresets[vertical];return <Link key={vertical} href={`/demo/${vertical}`} className="min-w-0 overflow-hidden rounded-2xl border border-neutral-200 hover:border-neutral-600"><div className="aspect-[4/3] min-w-0 overflow-hidden bg-neutral-100">{preset.imageUrl&&<Image unoptimized width={600} height={450} src={preset.imageUrl} alt={preset.product} className="block size-full max-w-full object-cover"/>}</div><div className="p-6"><h2 className="text-xl font-semibold">{preset.label}</h2><p className="mt-3 text-sm text-neutral-500">Выбрать оформление →</p></div></Link>})}</div>
