...
type TenantIntro = { name: string; slug: string; trial_ends_at: string; next_plan: Plan; business_vertical?: import("@/types/database").BusinessVertical | null };
export default async function OnboardingPage({ searchParams }: { searchParams: Promise<{ billing?: string }> }) {
  ...
  if (process.env.NEXT_PUBLIC_SUPABASE_URL && tenantId) { const found = (await getTenant(await createClient(), tenantId)).data; if (found) { tenant = { name: found.name, slug: found.slug, trial_ends_at: found.trial_ends_at ?? tenant.trial_ends_at, next_plan: found.next_plan ?? "standard", business_vertical: found.business_vertical }; initialBilling ??= found.preferred_billing_period === "annual" ? "year" : "month"; } }
