...
export async function GET(request:Request){
 ...
 try{
  ...
  const response=NextResponse.json({...history.data as Record<string,unknown>,signedIn:Boolean(verifiedUserId),phoneVerified,smsConsent:Boolean(consent?.data?.marketing_sms_consent)},{headers});setBuyerCookie(response,buyer.token);return response;
