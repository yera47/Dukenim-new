...
export async function POST(request:Request){
 ...
 try{
  ...
  const row=data[0];return NextResponse.json({orderNumber:row.order_number,total:row.total,expiresAt:row.expires_at,reservationStatus:row.reservation_status},{headers:{"Cache-Control":"private, no-store"}});
