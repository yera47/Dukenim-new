...
import {KaspiPayment} from "@/app/admin/orders/kaspi-payment";
import {CashRefund} from "../cash-refund";
...
export default async function RootOrderDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="min-h-screen bg-[var(--surface)] px-4 py-10"><div className="mx-auto max-w-4xl"><Link href="/root/orders" className="text-sm font-bold text-[var(--accent)]">← Все заказы</Link><h1 className="mt-5 text-3xl font-extrabold">Заказ № {order.order_number ?? order.id.slice(0,8)}</h1><p className="muted mt-2">{tenantResult.data?.name ?? "Магазин"} · {new Date(order.created_at).toLocaleString("ru-RU")}</p>
    ...
    {order.payment_method==="kaspi"&&order.status!=="cancelled"&&<KaspiPayment orderId={order.id} total={order.total} paid={order.payment_status==="paid"} refunded={order.payment_status==="refunded"} invoiceSent={Boolean(order.kaspi_invoice_sent_at)}/>}
    {order.payment_method==="cash"&&order.payment_status==="paid"&&order.order_number&&<CashRefund orderId={order.id} number={order.order_number} total={order.total}/>}
