...
export default async function RootFinancePage() {
  ...
  <div className="mt-6 grid gap-4 sm:grid-cols-3"><div className="card p-5"><b className="text-2xl">{subscriptionsResult.data?.filter(x=>x.status==="active"&&x.current_period_end&&Date.parse(x.current_period_end)>Date.now()).length??0}</b><p className="muted mt-2">Подписки со статусом active и неистёкшим периодом</p></div><div className="card p-5"><b className="text-2xl">{checkoutsResult.data?.filter(x=>x.status==="awaiting_payment").length??0}</b><p className="muted mt-2">Ожидают оплату тарифа</p></div><div className="card p-5"><b className="text-2xl">{crmResult.data?.filter(x=>x.status==="awaiting_payment").length??0}</b><p className="muted mt-2">Ожидают оплату CRM</p></div></div>
  <section className="card mt-6 overflow-x-auto"><h2 className="p-5 text-xl font-bold">Подписки</h2><table className="w-full min-w-[680px] text-left text-sm"><thead><tr className="border-b"><th className="p-4">Магазин</th><th>Тариф</th><th>Статус</th><th>Период до</th><th>Polar</th></tr></thead><tbody>{(subscriptionsResult.data??[]).map((row,index)=><tr key={`${row.tenant_id}-${index}`} className="border-b last:border-0"><td className="p-4">{names.get(row.tenant_id)??"Магазин удалён"}</td><td>{row.plan}</td><td>{row.status}{row.current_period_end&&Date.parse(row.current_period_end)<Date.now()?" · срок истёк":""}</td><td>{row.current_period_end?new Date(row.current_period_end).toLocaleDateString("ru-RU"):"—"}</td><td>{row.polar_subscription_id?"Связана":"Нет ID"}</td></tr>)}</tbody></table>{!subscriptionsResult.data?.length&&<p className="muted p-5">Подписок нет.</p>}</section>
