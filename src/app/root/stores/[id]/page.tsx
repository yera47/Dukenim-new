      ...
      <div className="container py-10">
        ...
        <section className="mt-8 grid gap-px overflow-hidden rounded-2xl bg-white/10 sm:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            ["Отмечено оплаченным · последние 20", money(revenue)],
          ].map(([label, value]) => (
        ...
        <p className="mt-3 text-xs text-white/55">Это сумма заказов по статусу в Dukenim, не выписка банка или Kaspi Pay.</p>
        <section className="mt-8 rounded-2xl bg-white/7 p-5"><h2 className="text-xl font-extrabold">Настройки и публикация</h2><p className="mt-1 text-sm text-white/55">Состояние магазина и управление видимостью витрины.</p><div className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4"><div><span className="text-white/50">Каталог</span><b className="mt-1 block">{tenant.catalog_published?"Опубликован":"Закрыт"}</b></div><div><span className="text-white/50">Доставка / самовывоз</span><b className="mt-1 block">{settings?.delivery_enabled?"Вкл.":"Выкл."} / {settings?.pickup_enabled?"Вкл.":"Выкл."}</b></div><div><span className="text-white/50">Онлайн-оплата</span><b className="mt-1 block">{settings?.payment_online?`Вкл. · ${settings.payment_provider??"провайдер не указан"}`:"Выкл."}</b></div><div><span className="text-white/50">Лояльность / истории</span><b className="mt-1 block">{loyaltyResult.data?.enabled?loyaltyResult.data.name:"Выкл."} / {(storiesResult.data??[]).filter(item=>item.status==="published").length} опубликовано</b></div></div><div className="mt-4 text-sm text-white/60">Зоны доставки: {(zonesResult.data??[]).filter(zone=>zone.is_active).map(zone=>`${zone.name} (${zone.provider}${zone.provider==="own"?`, ${money(zone.cost)}`:""})`).join(" · ")||"нет активных"}</div><details className="mt-5 rounded-xl border border-white/15 p-4"><summary className="cursor-pointer font-bold">{tenant.catalog_published?"Закрыть витрину":"Опубликовать витрину"}</summary><p className="mt-3 text-sm text-white/60">{tenant.catalog_published?"Покупатели перестанут видеть каталог. Данные и заказы сохранятся.":"Проверим действующий тариф, товар в наличии и настроенное получение заказа."} Изменение сохранится в аудите.</p><form action={setRootCatalogPublication} className="mt-4 grid gap-3 sm:max-w-lg"><input type="hidden" name="tenantId" value={tenant.id}/><input type="hidden" name="expected" value={String(tenant.catalog_published)}/><input type="hidden" name="publish" value={String(!tenant.catalog_published)}/><label className="text-sm font-semibold">Введите адрес магазина {tenant.slug}<input name="confirmSlug" required autoComplete="off" className="input mt-1 text-black"/></label><label className="text-sm font-semibold">Причина изменения<input name="reason" required minLength={3} maxLength={1000} className="input mt-1 text-black"/></label><button className="btn bg-white text-black">{tenant.catalog_published?"Закрыть с аудитом":"Опубликовать с аудитом"}</button></form></details></section>
