...
export function proposedDesignSettings(design:AiStudioDesign,current:{brand_color:string|null;hero_image_url:string|null}|null) {
  return {...(design.layout ? {layout_config:design.layout} : {}),...(design.colorTheme ? {color_theme:design.colorTheme} : {}),template_key:design.templateKey,palette_key:design.paletteKey,brand_color:design.brandColor??current?.brand_color??null,
