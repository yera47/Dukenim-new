...
export async function createAiStudioDesign(brief: string, vertical: BusinessVertical, plan: Plan) {
  ...
  if (!design.success || !allowedTemplates.includes(design.data.templateKey) || !allowedPalettes.includes(design.data.paletteKey)) throw new AzureFoundryError("AI Studio предложил недоступное оформление.");
