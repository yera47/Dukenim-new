...
export async function createAiStudioDesign(brief: string, vertical: BusinessVertical, plan: Plan, context: unknown = {}) {
  ...
  if (!allowedTemplates.includes(design.data.templateKey) || !allowedPalettes.includes(design.data.paletteKey)) throw new AzureFoundryError("AI Studio предложил недоступное оформление.",502,"design_entitlement");
