...
it.skipIf(process.env.RUN_AZURE_DESIGN_TEST!=="1")("actual deployment returns individual pink/green design",async()=>{
  // This test calls Azure only, never Supabase. Satisfy the application readiness
  // flag without exporting a real privileged database credential into the test.
  vi.stubEnv("SUPABASE_SERVICE_ROLE_KEY", "local-test-readiness-flag-not-a-credential");
  try {
