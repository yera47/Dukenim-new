...
export function authorizeWorker(headers: Headers, secret: string | undefined, scope: string, now = Date.now()): boolean {
  ...
  const expected = createHmac("sha256", secret).update(`dukenim:${scope}:${timestamp}`).digest("hex");
...
export function authorizePushWorker(headers: Headers, secret: string | undefined, now = Date.now()): boolean {
  return authorizeWorker(headers, secret, "mobile-push", now);
}
