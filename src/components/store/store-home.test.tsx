...
it.each(["compact","centered","editorial"] as const)("renders persisted personal %s layout through the common renderer",hero=>{
 ...
 const html=renderToStaticMarkup(<CartProvider><StoreHome slug="own" tenant={{name:"Серик",catalog_name:null,tagline:null,business_vertical:"fashion"}} products={demoProductsFor("fashion")} settings={settings} campaign={null} storePolicies={null}/></CartProvider>);
...
it.each(commerceConfigurations.filter(config=>config.approach==="assortment"&&config.vertical!=="food"))("offers real category shortcuts for $id",config=>{
 ...
 const html=renderToStaticMarkup(<CartProvider><StoreHome slug="own" tenant={{name:"Серик Шоп",catalog_name:null,tagline:null,business_vertical:config.vertical}} products={products} settings={null} campaign={null} storePolicies={null} approach="assortment"/></CartProvider>);
...
it("does not inject sample products into an empty real store",()=>{
  vi.stubGlobal("React",React);
  const html=renderToStaticMarkup(<CartProvider><StoreHome slug="own" tenant={{name:"Серик Шоп",catalog_name:null,tagline:null,business_vertical:"beauty"}} products={[]} settings={null} campaign={null} storePolicies={null}/></CartProvider>);
