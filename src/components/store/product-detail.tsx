...
export function ProductDetail({ product, slug, deliveryPolicy, returnPolicy }: { product: Product; slug: string; deliveryPolicy: string | null; returnPolicy: string | null }) {
  ...
  return <main className="container py-8">
    <Link href={`/s/${slug}`} className="muted inline-flex items-center gap-1 text-sm"><ChevronLeft size={17}/> Назад в каталог</Link>
    <div className="mt-7 grid gap-10 md:grid-cols-[1.1fr_.9fr]">
      ...
      <div className="md:p-5">
        ...
        <button type="button" onClick={() => { if (selected) { add(product, selected); setAdded(true); } }} disabled={!selected} className="btn btn-cta mt-8 w-full disabled:cursor-not-allowed disabled:opacity-50"><ShoppingBag size={19}/>{added ? <><Check size={18}/> Добавлено</> : "Добавить в корзину"}</button>
