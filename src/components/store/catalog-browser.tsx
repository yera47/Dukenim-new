...
export function CatalogBrowser({products,slug,food=false}:{products:Product[];slug:string;food?:boolean}) {
  ...
  return <div>
    {food&&categories.length>1&&<nav className="food-category-strip" aria-label="Разделы меню"><button type="button" aria-pressed={!filters.category} onClick={()=>change("category","")}>Всё меню</button>{categories.map(category=><button key={category} type="button" aria-pressed={filters.category===category} onClick={()=>change("category",category)}>{category}</button>)}</nav>}
    <div className="mb-4 grid grid-cols-[minmax(0,1fr)_auto] items-end gap-3 md:grid-cols-1">
      <label className="grid gap-2 text-sm" htmlFor={`${id}-search`}>{food?"Найти блюдо":"Найти товар"}<input id={`${id}-search`} type="search" value={filters.query} onChange={e=>change("query",e.target.value)} placeholder={food?"Название или состав":"Название или описание"} className={controls}/></label>
    ...
    <div id={`${id}-filters`} className={`${filtersOpen?"grid":"hidden"} mb-4 gap-3 sm:grid-cols-2 md:grid md:grid-cols-3`}>
      {!food&&categories.length>1&&<label className="grid gap-2 text-sm">Категория<select aria-label="Категория" value={filters.category} onChange={e=>change("category",e.target.value)} className={controls}><option value="">Все категории</option>{categories.map(category=><option key={category}>{category}</option>)}</select></label>}
    ...
    {visible.length?<div className="storefront-product-grid">{visible.map(product=>food?<FoodProductCard key={product.id} product={product} slug={slug}/>:<ProductCard key={product.id} product={product} slug={slug}/>)}</div>:<div className="rounded-2xl border border-dashed border-current/20 px-5 py-12 text-center"><h3 className="text-lg font-semibold">Ничего не найдено</h3><p className="mt-2 opacity-70">Измените запрос или сбросьте фильтры.</p></div>}
  </div>;
}
