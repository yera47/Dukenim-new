...
export function ProductCard({ product, slug }: { product: Product; slug: string }) {
  ...
  return <Link href={`/s/${slug}/product/${product.id}`} className="group block">
    ...
    <div className="pt-4"><div className="flex flex-col gap-2"><h3 className="min-w-0 break-words font-bold transition-opacity group-hover:opacity-70">{product.title}</h3><span className="whitespace-nowrap font-extrabold">{money(product.price)}</span></div><p className="mt-1 text-sm opacity-60">{product.variants[0]?.color === "Стандарт" ? product.category : product.variants[0]?.color}</p></div>
  </Link>;
}
