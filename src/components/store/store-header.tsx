...
export function StoreHeader({ slug, name, categories = [], demo = false, food = false, quickFood=false }: { slug: string; name: string; categories?: string[]; demo?: boolean; food?:boolean;quickFood?:boolean }) {
  ...
  return <header className="sticky top-0 z-30 border-b border-black/10 bg-[var(--store-surface)]">
    {demo && <div className="border-b border-black/10"><div className="container flex flex-wrap justify-between gap-3 py-3 text-xs"><Link href="/demo">← Другие примеры</Link><span className="hidden opacity-50 sm:inline">Демонстрационный каталог</span><Link href="/register" className="font-semibold">Создать магазин →</Link></div></div>}
