...
export function StoreHeader({ slug, name, categories = [], demo = false, food = false, quickFood=false }: { slug: string; name: string; categories?: string[]; demo?: boolean; food?:boolean;quickFood?:boolean }) {
  ...
  if(quickFood)return <header className="sticky top-0 z-30 border-b border-black/5 bg-white"><div className="mx-auto flex h-[72px] max-w-[1180px] items-center justify-between gap-3 px-4 sm:px-6"><Link href={base} className="min-w-0 truncate text-xl font-extrabold tracking-tight text-[#6650cf]">{name}</Link><div className="flex shrink-0 items-center gap-3">{!demo&&<Link className="text-xs text-neutral-500" href={`${base}/orders`}>Мои заказы</Link>}<Link aria-label={`Корзина, ${count} товаров`} className="flex items-center gap-2 rounded-full bg-[#6650cf] px-4 py-3 text-sm font-semibold text-white" href={`${base}/cart`}><ShoppingBag size={18}/><span>{count||"Корзина"}</span></Link></div></div></header>;
