...
export function StoreHeader({ slug, name, categories = [], demo = false }: { slug: string; name: string; categories?: string[]; demo?: boolean }) {
  ...
  const links = [{ label: "Все товары", href: `${base}/catalog` }, ...categories.map(label => ({ label, href: `${base}/category/${encodeURIComponent(label)}` })), ...(!demo?[{label:"Мои заказы",href:`${base}/orders`}]:[])];
