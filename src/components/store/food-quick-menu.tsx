...
export function FoodQuickMenu({products,slug,name,curatedStories}:{products:Product[];slug:string;name:string;curatedStories?:FoodStory[]}) {
  ...
  const stories:FoodStory[]=curatedStories??products.filter(product=>product.images?.[0]).slice(0,6).map(product=>({id:product.id,title:product.title,caption:product.category,mediaUrl:product.images![0],mediaType:"image" as const,productId:product.id}));
  ...
  return <div className={styles.root}>
    {stories.length>0&&<section className={styles.stories} aria-label="Истории кафе">{stories.map((item,index)=><button key={item.id} className={styles.storyTile} aria-label={`Открыть историю: ${item.title}`} onClick={event=>{trigger.current=event.currentTarget;setStory(index);}}>
      {/* eslint-disable-next-line @next/next/no-img-element */}
      {item.mediaType==="video"?<video src={item.mediaUrl} muted playsInline preload="metadata"/>:<img src={item.mediaUrl} alt=""/>}<span>{item.title}</span><small>{item.mediaType==="video"?"▶ Видео":"Смотреть ↗"}</small>
    </button>)}</section>}
