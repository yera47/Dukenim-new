...
export function CrmSetupPayment({chargeId}:{chargeId:string}){
  ...
  return <div className="mt-3 rounded-xl border border-amber-300 bg-amber-50 p-4 text-amber-950"><b>Подключение проверено</b><p className="mt-1 text-sm">Настройка со стороны Dukenim входит в тариф «Каталог». Лицензия поставщика оплачивается отдельно.</p><button type="button" className="btn btn-cta mt-3" disabled={pending} onClick={pay}>{pending?"Открываем оплату…":"Продолжить"}<ArrowRight size={16}/></button>{error&&<p role="alert" className="mt-2 text-sm">{error}</p>}</div>;
}
