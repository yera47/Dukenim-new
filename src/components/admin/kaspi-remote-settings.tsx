...
export function KaspiRemoteSettings({enabled,link}:{enabled:boolean;link:string|null}){
 ...
 return <form action={action} className="mt-5 space-y-4 rounded-2xl border border-sky-200 bg-sky-50/50 p-5">
  ...
  <label className="block text-sm font-semibold">Ссылка для оплаты · необязательно<input name="link" type="url" defaultValue={link??""} className="input mt-2" placeholder="https://…kaspi.kz/…" autoComplete="url"/><small className="mt-1 block font-normal text-neutral-600">Без ссылки вы сможете выставлять счёт покупателю по его номеру из Kaspi Pay.</small></label>
