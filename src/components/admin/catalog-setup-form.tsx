    ...
    <div className={fromStudio ? "space-y-4" : "catalog-wizard-layout"}>
      <fieldset key={step} disabled={pending || aiPending || draftLoading || draftSaving || brandBusy} className="catalog-active-step min-w-0 py-4">
        ...
        {step===4&&vertical==="food"&&<div className="mt-5 space-y-4"><p className="text-sm text-neutral-500">Программу можно настроить сейчас или позже в разделе «Настройки → Лояльность». Без неё магазин и заказы работают как обычно.</p>{loyalty.enabled?<LoyaltyEditor value={loyalty} onChange={setLoyalty}/>:<button type="button" className="btn btn-secondary" onClick={()=>setLoyalty({...loyalty,enabled:true,rules:loyalty.rules.map(rule=>rule.reward==="gift"&&!rule.giftVariantId?{...rule,id:crypto.randomUUID(),reward:"percent" as const,value:10,label:"Скидка 10% на следующий заказ"}:rule)})}>Добавить программу лояльности</button>}</div>}{step===2&&<div className="mt-5 space-y-5">
          ...
          {fulfilment.delivery&&<div className="space-y-4 rounded-2xl border border-neutral-200 bg-white p-4">
            <fieldset><legend className="mb-2 text-sm font-semibold">Кто доставит заказ?</legend><div className="grid gap-2 sm:grid-cols-2">
              <label className={`rounded-xl border p-3 text-sm ${fulfilment.deliveryProvider==="own"?"border-neutral-900 bg-neutral-50":"border-neutral-200"}`}><input type="radio" name="delivery-provider" checked={fulfilment.deliveryProvider==="own"} onChange={()=>setFulfilment({...fulfilment,deliveryProvider:"own",cost:fulfilment.cost==="0"?"":fulfilment.cost})}/> <b>Своя доставка</b><span className="mt-1 block text-xs text-neutral-500">Вы назначаете цену для покупателя заранее.</span></label>
              <label className={`rounded-xl border p-3 text-sm ${fulfilment.deliveryProvider==="yandex"?"border-neutral-900 bg-neutral-50":"border-neutral-200"}`}><input type="radio" name="delivery-provider" checked={fulfilment.deliveryProvider==="yandex"} onChange={()=>setFulfilment({...fulfilment,deliveryProvider:"yandex",cost:"0"})}/> <b>Курьер через Яндекс · вручную</b><span className="mt-1 block text-xs text-neutral-500">Вы сами закажете курьера после согласования цены с покупателем.</span></label>
            </div></fieldset>
            <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm">Куда доставляете<input className="input mt-2" value={fulfilment.zone} maxLength={100} placeholder="Например, Алматы в пределах города" onChange={e=>setFulfilment({...fulfilment,zone:e.target.value})}/></label><label className="text-sm">Срок доставки<input className="input mt-2" value={fulfilment.eta} maxLength={200} placeholder="Например, в течение дня" onChange={e=>setFulfilment({...fulfilment,eta:e.target.value})}/></label>
              {fulfilment.deliveryProvider==="own"&&<label className="text-sm sm:col-span-2">Цена для покупателя, ₸<input className="input mt-2" value={fulfilment.cost} inputMode="numeric" maxLength={10} placeholder="Например, 1500" onChange={e=>setFulfilment({...fulfilment,cost:e.target.value})}/></label>}
            </div>
            {fulfilment.deliveryProvider==="yandex"&&<p className="rounded-xl bg-blue-50 p-3 text-sm leading-6 text-blue-950">Покупатель укажет телефон и адрес, подтвердит, что цена доставки зависит от расстояния, и оформит заказ без заранее указанной цены доставки. После заказа вы свяжетесь с ним, согласуете сумму и сами вызовете курьера от двери до двери. Аккаунт Яндекса к Dukenim подключать не нужно.</p>}
          </div>}
        ...
        {step===3&&<div className="mt-5 space-y-3">
          <p className="mb-4 text-sm text-neutral-500">Выберите способ. Удалённый счёт Kaspi Pay можно выставлять вручную уже после создания магазина; эквайринг картами требует отдельного подключения.</p>
          {([['later','Подключу оплату позже','Продолжить создание магазина без удалённой оплаты.'],['kaspi','Удалённая оплата через Kaspi Pay','Покупатель выберет Kaspi при заказе. Вы отправите счёт на его телефон или ссылку вручную, сверите поступление в Kaspi Pay и подтвердите оплату в заказе. Автоматического списания нет.'],['freedompay','Карты через FreedomPay','Заявка на подключение эквайринга: платёжная страница и подтверждение серверу появятся после настройки.'],['halyk','Карты через Halyk ePay','Заявка на эквайринг Halyk: нужны реквизиты доступа от банка.']] as const).map(([value,label,description])=><label key={value} className={`flex gap-3 rounded-2xl border p-4 ${paymentPreference===value?'border-neutral-900 bg-white':'border-neutral-200'}`}><input type="radio" name="payment-choice" value={value} checked={paymentPreference===value} onChange={()=>setPaymentPreference(value)}/><span><strong className="block text-sm">{label}</strong><span className="mt-1 block text-xs leading-5 text-neutral-500">{description}</span></span></label>)}
        </div>}
        {step===reviewStep&&<dl className="mt-5 space-y-2 rounded-xl border p-4 text-sm"><div><dt className="text-neutral-500">Получение</dt><dd>{fulfilment.delivery?<p>Доставка: {fulfilment.zone} · {fulfilment.deliveryProvider==="yandex"?"Яндекс вручную, цена после согласования":`${fulfilment.cost} ₸`} · {fulfilment.eta}</p>:null}{fulfilment.pickup?<p>Самовывоз: {fulfilment.address}, {fulfilment.hours}</p>:null}{fulfilment.reservation?<p>Бронь: {fulfilment.address}, {fulfilment.holdHours} ч.</p>:null}</dd></div><div><dt className="text-neutral-500">Оплата</dt><dd>{paymentPreference==='later'?'Подключить позже':paymentPreference==='kaspi'?'Kaspi Pay · удалённый счёт от владельца':`${paymentPreference} · заявка на подключение`}</dd></div></dl>}
