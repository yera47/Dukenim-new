...
export function LoyaltyEditor({value,onChange,gifts=[]}:{value:LoyaltyProgram;onChange:(value:LoyaltyProgram)=>void;gifts?:LoyaltyGiftOption[]}) {
 ...
 return <section className={styles.editor} aria-label="Конструктор лояльности">
  ...
  <div className={styles.rule}>
   ...
   <label><b>2. Что получает гость</b><select value={rule.reward} onChange={e=>{const reward=e.target.value as LoyaltyRule["reward"];change({reward,value:reward==="gift"?1:10,giftVariantId:reward==="gift"?(gifts[0]?.variantId??null):null,...(reward==="cashback"?{trigger:"orders",threshold:1}:{}),label:reward==="gift"?"Подарок из меню":reward==="cashback"?"Кешбэк на следующий заказ":"Скидка на следующий заказ"});}}><option value="gift">Подарок</option><option value="percent">Скидка в процентах</option><option value="fixed">Скидка в тенге</option><option value="cashback">Кешбэк с каждого заказа</option></select></label>
