...
export type LoyaltyGiftOption={variantId:string;title:string;detail:string};
export function LoyaltyEditor({value,onChange,gifts=[]}:{value:LoyaltyProgram;onChange:(value:LoyaltyProgram)=>void;gifts?:LoyaltyGiftOption[]}) {
 ...
 return <section className={styles.editor} aria-label="Конструктор лояльности">
  ...
  <div className={styles.rule}>
   ...
   {rule.trigger==="referral"&&<p className={styles.note}>Ссылка «Пригласить друга» появится у вошедшего гостя в «Моей карте». Друг откроет по ней меню, оформит первый заказ и войдёт в аккаунт. Награда засчитается после оплаты и выдачи заказа.</p>}
   ...
   <label><b>2. Что получает гость</b><select value={rule.reward} onChange={e=>{const reward=e.target.value as LoyaltyRule["reward"];change({reward,value:reward==="gift"?1:10,giftVariantId:reward==="gift"?(gifts[0]?.variantId??null):null,...(reward==="cashback"?{trigger:"orders",threshold:1}:{}),label:reward==="gift"?"Подарок из меню":reward==="cashback"?"Кешбэк на следующий заказ":"Скидка на следующий заказ"});}}><option value="gift" disabled={!gifts.length}>Подарок {gifts.length?"":"· сначала добавьте товар"}</option><option value="percent">Скидка в процентах</option><option value="fixed">Скидка в тенге</option><option value="cashback">Кешбэк с каждого заказа</option></select></label>
   ...
   {rule.reward==="gift"&&<label>Товар-подарок<select value={rule.giftVariantId??""} onChange={event=>change({giftVariantId:event.target.value||null})}><option value="">Выберите товар из меню</option>{gifts.map(gift=><option key={gift.variantId} value={gift.variantId}>{gift.title}{gift.detail?` · ${gift.detail}`:""}</option>)}</select><small>{gifts.length?"При использовании награды товар автоматически добавится в заказ бесплатно и спишется со склада.":"Сначала добавьте товар в каталог, затем вернитесь сюда и свяжите подарок с товаром."}</small></label>}
