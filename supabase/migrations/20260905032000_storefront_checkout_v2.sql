...
begin
  ...
  if not found then
    v_delivery_enabled := false;
    v_pickup_enabled := true;
    v_min_order := 0;
  end if;
  ...
  if p_delivery_method = 'courier' then
    select cost, free_from into v_zone
  ...
  values (p_tenant_id, v_customer_id, 'online', 'new', p_delivery_method::public.delivery_method, nullif(trim(p_delivery_address), ''), v_delivery_cost, v_subtotal, v_subtotal + v_delivery_cost, p_payment_method::public.payment_method, 'pending')
