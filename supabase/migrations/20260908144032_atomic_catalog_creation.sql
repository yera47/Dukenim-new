...
create function public.create_catalog_atomic(p_tenant_id uuid,p_name text,p_template text,p_palette text,p_brand_color text default null)
...
begin
  ...
  if not (v_tenant.status='active' or (v_tenant.status='trial' and v_tenant.trial_ends_at>now())) then raise exception 'Entitlement inactive'; end if;
  ...
  if p_template is null or not ((v_plan='basic' and p_template in ('atelier','market')) or (v_plan in ('standard','pro') and p_template in ('journal','gallery'))) then raise exception 'Template unavailable'; end if;
