-- The public product now has one paid plan. Legacy plan values remain on
-- tenants for compatibility, but they no longer restrict storefront design.
create or replace function public.create_catalog_atomic(
  p_tenant_id uuid,
  p_name text,
  p_template text,
  p_palette text,
...
begin
  ...
  if (v_tenant.status='active' or (v_tenant.status='trial' and v_tenant.trial_ends_at>now())) is not true then
    raise exception 'Entitlement inactive';
  end if;
  if length(trim(coalesce(p_name,''))) not between 2 and 80 then raise exception 'Invalid catalog name'; end if;
  if p_template is null or p_template not in ('atelier','market','studio','journal','gallery','signature') then
    raise exception 'Template unavailable';
  end if;
  if p_palette is null or p_palette not in ('mono','ink-brass','paper-forest','clay-milk','ocean-sand','plum-stone','cobalt-cloud','olive-linen','cherry-cream','terra-charcoal','mint-charcoal','rose-ink','sunset-navy') then
    raise exception 'Invalid palette';
  end if;
  if p_brand_color is not null and p_brand_color !~ '^#[0-9a-fA-F]{6}$' then raise exception 'Invalid brand color'; end if;
  ...
  set catalog_name=trim(p_name),catalog_status='building',catalog_created_at=now()
