...
## 2026-09-21 — Чистый старт магазинов владельца

Владелец попросил удалить созданные им магазины, оставить самостоятельное удаление в панели и затем самому пройти создание нового магазина с нуля. Реальные товары и цифры продаж добавлять только после его следующей просьбы. Удаление было ограничено двумя магазинами его `ersatik@gmail.com` аккаунта; чужие магазины платформы и аккаунт владельца сохраняются.

## 2026-09-21 — Полное управление только для владельца платформы

Ерсат уточнил, что карта полного управления всеми магазинами и план недостающих root-функций предназначены только ему как владельцу Dukenim, а не продавцам или покупателям. Список возможностей размещается в защищённом `/root/management`; в обычном `/stores` остаются лишь магазины конкретного аккаунта и собственные действия владельца магазина.

## 2026-09-22 — Удалённая оплата и оформление платформы

Владелец выбрал ручную удалённую оплату через Kaspi Pay: продавец выставляет счёт по телефону или даёт ссылку, покупатель видит точную сумму заказа, а продавец сверяет фактическое поступление перед подтверждением и выдачей. Автоматическое списание Dukenim без работающей платёжной интеграции не обещается. SMS остаётся отложенным. Основной интерфейс платформы получает тёмно-синий акцент; у знака D чёрная буква, синий порог и точка.

- 2026-09-23: Логотип приложения и сайта обязан использовать точную геометрию исходного знака D из брендбука. Нельзя перерисовывать или менять пропорции знака; цветовая правка касается только нижнего порога: буква остаётся тёмной на светлом фоне или белой на тёмном, порог и точка — голубые.
- 2026-09-24: Голубой акцент логотипа отменён. Точная геометрия D сохраняется; только плоский порог двери и точка слова получают единый тёмно-синий `#173B57`. Этот же тёмно-синий заменяет голубой акцент в интерфейсе сайта и мобильного приложения.
- 2026-09-24: Нижний порог внутри буквы D должен быть белым, без голубого или синего заполнения. Геометрия D остаётся неизменной. Фон мобильного приложения, splash screen, виджета и иконки должен быть чисто белым; тёмно-синий остаётся интерфейсным цветом кнопок и навигации.
- 2026-09-24: Белый порог сливается с белым фоном, поэтому финальный вариант логотипа — чёрная D на белом фоне с видимым порогом среднего тёмно-синего `#315F78`. Этот оттенок заменяет более тёмный `#173B57` в активных интерфейсных акцентах; чисто белый фон сохраняется.

## 2026-09-22 — Единый путь Kaspi Pay и доставка через Яндекс

Если магазин включил ручную удалённую оплату Kaspi Pay, покупателю показывается один способ оплаты товаров: заказ создаётся в Dukenim, покупатель видит его в «Моих заказах» и самостоятельно вводит показанную сумму в приложении Kaspi по ссылке магазина либо получает счёт от продавца по телефону. Переход в Kaspi не подтверждает оплату; продавец сверяет поступление и меняет статус вручную. При Яндекс-доставке стоимость курьера отдельно согласуется с покупателем, после чего владелец сам оформляет курьера; цена не добавляется к сумме товаров заранее.

## 2026-09-24 — Один тариф «Каталог»

Владелец отменил публичное деление на «Старт» и «Бизнес/Бренд» и годовую оплату. В Dukenim остаётся один публичный тариф «Каталог» за 24 900 ₸ в месяц с семью бесплатными днями и всеми функциями магазина. Старые значения тарифов сохраняются только внутри базы для совместимости существующих записей и не должны ограничивать функции или показываться продавцам.
