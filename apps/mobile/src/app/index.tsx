...
export default function HomeScreen() {
  ...
  const signIn = async () => {
    ...
    if (!email.trim() || !password) {
      Alert.alert("Заполните данные", "Укажите email и пароль от кабинета Dukenim.");
      return;
    }
    setSigningIn(true);
    const { error } = await supabase.auth.signInWithPassword({ email: email.trim(), password });
    setSigningIn(false);
    if (error) Alert.alert("Не удалось войти", "Проверьте email, пароль и интернет."); else setPassword("");
  };
  const signOut = async () => {
    try {
  ...
  return <SafeAreaView style={styles.screen}><ScrollView contentContainerStyle={styles.content}>
    ...
    {!isSupabaseConfigured ? <View style={styles.card}><Text style={styles.cardTitle}>Нужна настройка среды</Text><Text style={styles.muted}>Добавьте публичный URL и publishable key Supabase в EXPO_PUBLIC_* переменные. Секреты и service-role ключи в приложение не попадают.</Text></View> : user ? <>
      ...
      <View style={styles.grid}><Link href="/orders" asChild><Pressable style={styles.card}><Text style={styles.cardTitle}>Заказы</Text><Text style={styles.muted}>Новые и текущие заказы</Text><Text style={styles.cardAction}>Открыть →</Text></Pressable></Link><Link href="/scanner" asChild><Pressable style={styles.card}><Text style={styles.cardTitle}>Сканер</Text><Text style={styles.muted}>Штрихкод камерой</Text><Text style={styles.cardAction}>Сканировать →</Text></Pressable></Link></View>
