...
export default function CatalogScreen() {
  ...
  const publish=()=>{if(!store||!supabase)return;Alert.alert("Опубликовать магазин?","Dukenim проверит тариф, активный товар с остатком и способ получения заказа.",[{text:"Отмена",style:"cancel"},{text:"Опубликовать",onPress:async()=>{const{error}=await supabase!.rpc("publish_catalog",{p_tenant_id:store.id});if(error)Alert.alert("Публикация пока недоступна","Проверьте активный товар с остатком, доставку или самовывоз и действующий тариф.");else{await load();Alert.alert("Магазин опубликован","Ссылка витрины доступна покупателям.");}}}]);};
  return <SafeAreaView style={styles.page}><ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>{setRefreshing(true);void load(true);}} tintColor={colors.navy}/>}>
    <View style={styles.top}><Link accessibilityLabel="Назад" href="/studio" style={styles.back}>‹</Link><Text style={styles.kicker}>КАТАЛОГ</Text></View>
    ...
    {store&&<View style={styles.storeBar}><View><Text style={styles.storeName}>{store.name}</Text><Text style={styles.storeMeta}>{store.catalog_published?"Опубликован":"Виден только вам"} · {products.length} поз.</Text></View><Link href={{pathname:"/product-new",params:{tenantId:store.id,tenantName:store.name,vertical:store.business_vertical??"other"}} as never} asChild><Pressable style={styles.add}><Text style={styles.addText}>＋ Добавить</Text></Pressable></Link></View>}
    {store&&!store.catalog_published&&products.length>0?<Pressable onPress={publish} style={styles.publish}><View style={{flex:1}}><Text style={styles.publishTitle}>Магазин пока виден только вам</Text><Text style={styles.publishCopy}>Когда всё готово, откройте витрину покупателям.</Text></View><Text style={styles.publishButton}>Опубликовать</Text></Pressable>:null}
