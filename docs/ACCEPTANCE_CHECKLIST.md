...
Обновлено 2026-09-08. Статусы: не начат / в работе / проверен / внешне заблокирован. «В работе» не означает готовность. Прежние исследования и релизы учитываются как частичные доказательства, а не как закрытие требований.
...
| 03 | Логотип, необязательный брендбук, референсы | не начат | Нет полного upload → analysis → confirmed design пути |
| 04 | Индивидуальная композиция/цвет/шрифт | в работе | Есть enum тем/палитр, не требуемая персонализация |
| 05 | Ручные и AI-правки после запуска | в работе | Формы и apply существуют; нет revision/rollback/conflict E2E |
...
| 10 | Самовывоз, адрес/часы, карта/2ГИС/Яндекс | не начат | Не выводить выдуманный адрес или бесплатный API без проверки условий |
...
| 13 | Заказ и неизменный snapshot условий | в работе | Checkout error handling опубликован, 106 tests; DB и authenticated scenario нужны |
| 14 | Общий renderer preview/published | не начат | NichePreview отдельный и демонстрационный |
| 15 | Azure модель/контекст/tools/E2E | в работе | Живой createConsultation → Kimi-K2.6 с историей прошёл проверку JSON (8.6с). Полная цепочка browser message → confirmed saved shop change ещё не проверена; анализ файлов не поддержан этим тестом |
| 16 | Azure расходы и opt-out | внешне заблокирован | CLI вход завершён, ресурс/модель Succeeded. Azure PATCH отвечает NoAutoUpgrade, но повторный GET возвращает OnceUpgradeIsAvailable (три поддерживаемые версии API). Отключение не подтверждено; нужна проверка согласованности/разбор Microsoft, не новый вход владельца. Текущий tier Free Tier не означает бесплатный inference |
...
| 20 | Native notifications/order deep link | в работе | Исходники push есть; device E2E/production delivery не подтверждены |
