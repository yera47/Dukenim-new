...
| 02 | Полный пошаговый AI-диалог | в работе | Только intent/brief и трёхшаговый мастер; нужен реальный сохраняемый разговор |
...
| 15 | Azure модель/контекст/tools/E2E | в работе | Историческое подключение есть; новая цепочка message → saved change не проверена |
| 16 | Azure расходы и opt-out | внешне заблокирован | Исправлен каталог портала на fc24c2a2…: ресурс Succeeded. CLI installer ждёт подтверждения Windows (consent.exe). Подготовлен проверяющий GET/PATCH/GET скрипт, но политика не применена. Не обещать бесплатный inference |
