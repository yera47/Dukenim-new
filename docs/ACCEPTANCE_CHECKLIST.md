...
17:48 уточнение: эквайринг отложен владельцем. ID24: отдельные маршруты18конфигураций и корзины реализованы;36browser journeys390/1440прошли, но композиции ещё используют общие renderer-компоненты. ID27: приватные текущие себестоимости и неизменные snapshotпри создании заказа, stockформа и gross-profit отчёт реализованы; миграция применена, SQLowner/read/write/outsider/snapshot тесты пройдены. Authenticated UI/relogin ещё не проверен. Остальные открытые пункты не закрываются этим этапом.
...
| 12 | Бронь, атомарный hold/expiry/cancel | в работе | 10.09: отдельные settings/таблица/RPC и интерфейсы мастера, гостя и владельца реализованы. Пять миграций применены; rollback fixture проверяет replay, tenant isolation, резерв без обычного pickup, выдачу/оплату/возврат остатков ровно один раз, запрет oversell. Cron expiry succeeded. Полный браузерный сценарий брони ещё не пройден, source не опубликован |
