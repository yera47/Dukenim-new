...
| 03 | Логотип, необязательный брендбук, референсы | в работе | Приватный raster upload, извлечение цветов и текстовые правила для диалога; schema/API/image/DB isolation tests пройдены. PDF/визуальные референсы, Storage browser и confirmed design путь ещё не готовы |
...
| 20 | Native notifications/order deep link | в работе | Исправлена и применена отсутствовавшая очередь/токены; order RPC transaction проверяет создание обезличенного уведомления. Worker проверяет ответ Expo/права. Нет scheduler, receipt polling, lease recovery и native login/deep link/device E2E |
