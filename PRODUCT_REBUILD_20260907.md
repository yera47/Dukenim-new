...
5. Root workspace: tenant-scoped product editing and reversible hide/restore are now implemented with mandatory reason and fail-closed before/after audit. Support context and incident diagnostics already exist. Remaining: tenant search/filter, explicit recovery UI, richer truthful analytics and category/storefront-setting management. Owner credentials, payment state and immutable order/stock history stay inaccessible; no unbounded delete or arbitrary SQL controls.
